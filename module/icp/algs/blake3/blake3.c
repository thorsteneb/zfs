...
INLINE size_t compress_chunks_parallel(const uint8_t *input, size_t input_len,
  ...
  ASSERT3U(0, <, input_len);
  ASSERT3U(input_len, <=, MAX_SIMD_DEGREE * BLAKE3_CHUNK_LEN);
#endif
  ...
  blake3_hash_many(chunks_array, chunks_array_len,
                   ...
                   B_TRUE, flags, CHUNK_START, CHUNK_END, out);
...
INLINE size_t compress_parents_parallel(const uint8_t *child_chaining_values,
                                        ...
                                        uint8_t *out) {
#if defined(BLAKE3_TESTING)
  ASSERT3U(2, <=, num_chaining_values);
  ASSERT3U(num_chaining_values, <=, 2 * MAX_SIMD_DEGREE_OR_2);
#endif
...
size_t blake3_compress_subtree_wide(const uint8_t *input, size_t input_len,
  ...
  uint8_t *cv_array = kmem_alloc(2 * MAX(degree, 2) * BLAKE3_OUT_LEN, KM_SLEEP);
  ...
  if (left_n == 1) {
    ...
    kmem_free(cv_array, 2 * MAX(degree, 2) * BLAKE3_OUT_LEN);
    return 2;
  }

  // Otherwise, do one layer of parent node compression.
  size_t num_chaining_values = left_n + right_n;
  size_t ret = compress_parents_parallel(cv_array, num_chaining_values, key, flags,
                                   out);
  kmem_free(cv_array, 2 * MAX(degree, 2) * BLAKE3_OUT_LEN);
  return ret;
}
...
#if defined(BLAKE3_TESTING)
  ASSERT3U(input_len, >, BLAKE3_CHUNK_LEN);
#endif
...
#ifdef _KERNEL
EXPORT_SYMBOL(blake3_hasher_init);
EXPORT_SYMBOL(blake3_hasher_update);
EXPORT_SYMBOL(blake3_hasher_finalize);
#endif
